...
import { getStorePath, inferEmailFromOAuth, sameAccountIdentity, sameOAuthRecord } from "./utils/auth.ts";
...
export function syncCurrentAuth(
  ...
  if (existing) {
    ...
    if (store.activeAccountID === existing.id && existing.email && sameOAuthRecord(existing.auth, auth)) {
      return store;
    }
